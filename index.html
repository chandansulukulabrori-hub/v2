...
/* ---------- Playlist musik latar (2 lagu, muter bergantian) ---------- */
// Nama file dibungkus encodeURI() supaya spasi di "About You.mp3" tetap aman
// dibuka di semua browser (iOS Safari & Chrome termasuk yang cerewet soal ini).
const PLAYLIST = [
    { title: "Beautiful", src: encodeURI("Beautiful.mp3") },
    { title: "About You", src: encodeURI("About You.mp3") }
];
...
musicNextBtn.addEventListener("click", () => playTrack(nextTrackIndex(), true));
...
/* ---------- Audio: playlist 2 lagu + sfx + mute ---------- */

function nextTrackIndex() {
    return (currentTrackIndex + 1) % PLAYLIST.length;
}

// Muat & (opsional) langsung mainkan track pada index tertentu.
// Dipanggil pas unlock pertama kali, pas klik tombol next, dan pas satu lagu selesai.
function playTrack(index, autoplay) {
    ...
    if (autoplay) {
        bgMusic.play().catch(() => {
            // Browser blokir autoplay di luar gesture, atau file asetnya belum ada.
        });
        musicPlayBtn.textContent = "🎵";
    }
}

// Lagu lanjut otomatis ke berikutnya begitu satu track selesai, muter terus 2 lagu bergiliran.
...
function resetAndReplay() {
    ...
    // Musik latar sengaja TIDAK direset — biar lagu tetap lanjut sebagai ambiance,
    // gak keputus cuma karena cerita di-replay dari awal.
}
